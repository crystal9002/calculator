var XMath = function() {
	...
	_this.recursion = function(args, callback, initValue) {
		...
		initValue = callback(initValue, args[0] * 1.0);
		args = args.slice(1);
		return _this.recursion(args, callback, initValue);
	};
...
function NodeBuilder() {
	...
	this.bracketsParse = function(str) {
		...
		for(var i in matches) {		
			...
			str = str.replace('(' + matches[i] + ')', this.mathParse(matches[i]));
		}
	...
	this.mathParse = function(str) {
		...
		for(var i in res) {
			...
			if(i % 2 == 0) {
				if(isNaN(res[i].value)) {
					throw new Error('wrong number:' + res[i].value);
				}
				continue;
			}
			// 奇数下标的为操作符
		...
		return res[0].getRoot().calculate();
	};
};
// ((1+2)+((5*6)-7)+(3/4)) = 26.75
var str = "(1+2)+(5*6-7)+(3/4)";
var nodeBuilder = new NodeBuilder();
var root = nodeBuilder.parse(str);
console.log(root);